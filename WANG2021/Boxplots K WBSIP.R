...
getDist<-function(thresh_wbs=20){
  ...
  summ_wbs<-getSummaryK(results_wbs,N,theta)
  # summ_bs<-getSummaryK(results_bs,N,theta)
  return(summ_wbs)
...
# 
  if(d==10&&distr==1){
    ...
    thresh_wbs=561
    vals<-getDist(thresh_wbs)
    
    output_wbs<-cbind(output_wbs,vals)
...
Cairo::CairoPDF(file="kbp_wbs.pdf",height=6,width=10)
makeBP(output_wbs)
dev.off()
...
Cairo::CairoPDF(file=paste("kbp_WBSIP_1000.pdf",sep=""),height=7.5,width=14)
makeBigPlot(output[,order(dimss)])
title("N=1000, Scenario 1, WBSIP",cex.main=2.5)
dev.off()