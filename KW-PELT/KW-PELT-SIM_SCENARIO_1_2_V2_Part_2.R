...
#   closeAllConnections()
  ...
  ## Ran this already, but should run if rerunning
  
  constants<-c(seq(0.005,0.05,l=10))
  constants=unique(round(constants,2))


  for(j in 132:numUniqueRuns){
    params<-paramterIndices[j,]
    N=Ns[params[1]]
    # beta=constant*sqrt(N)+3.74
    ...
    distr=distributions[[distr]]
    for(constant in constants){
      fileName<-paste0(N,"_",length(theta),"_",dName,"_",d,"_new_constant_C1_",constant,"OC_PELT_ranks_simsize_",sim.size,sep="")
      fileName1<-paste(dirr2,fileName,".Rda",sep="")
      if(!file.exists(fileName1)){
        beta=constant*max(c(log(N),d*log(N/d)))*sqrt(N)
