...
# constants<-c(seq(0.05,1.1,l=20))
# constants=round(constants,2)
# # constants<-c(seq(0.005,0.05,l=10))
# # constants=round(constants,2)
# constants<-c(seq(0.005,0.05,l=10))
# constants=unique(round(constants,2))
...
#     beta=constant*max(c(log(N),d*log(N/d)))*sqrt(N)
# 
# 
#     fileName<-paste0(N,"_",length(theta),"_",dName,"_",d,"_","OC_SS_Depths_simsize_",sim.size,sep="")
# 
# 
...
# 
#     fileName<-paste0(N,"_",length(theta),"_",dName,"_",d,"_new_constant_C1_",constant,"SS_PELT_ranks_simsize_",sim.size,sep="")
...
for(j in 1:numUniqueRuns){
  for(constant in constants){
    ...
    fileName<-paste0(N,"_",length(theta),"_",dName,"_",d,"_constant_C1_",constant,"SS_PELT_ranks_simsize_",sim.size,sep="")
    fileName1<-paste(dirr2,fileName,".Rda",sep="")
    save(resultsHS,resultsSpat,resultsMahal,resultsMahal75,file=fileName1)
