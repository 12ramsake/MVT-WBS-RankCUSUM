...
#results dirrectory
dirr<- "C:/Users/12RAM/OneDrive/Documents/research/PhD Thesis/MKWC changepoint/WBS_SIM/Small_Sample/"
setwd(dirr)
...
for(j in 1:numUniqueRuns){
  ...
  # 
  # 
  # resultsHS=try({foreach(i=1:sim.size,.packages= pkgs) %do% {runSim(N,theta,distr,d,numInt,thresh=thresh,depth="hs")}})
  # 
  ...
  resultsSpat=try({foreach(i=1:sim.size,.packages= pkgs) %do% {runSim(N,theta,distr,d,numInt,thresh=thresh,depth="spat")}})
  ...
  # ############################################# M ############################################# #############################################
  # 
  # resultsMahal=try({foreach(i=1:sim.size,.packages= pkgs) %do% {runSim(N,theta,distr,d,numInt,thresh=thresh,depth="mahal")}})
  # 
  ...
  # 
  # 
  # resultsMahal75=try({foreach(i=1:sim.size,.packages= pkgs) %do% {runSim(N,theta,distr,d,numInt,thresh=thresh,depth="mahal75")}})
  # 
